  ...
  setFrame: {
    value: function setFrame(frameId) {
      if (frameId >= 0 && frameId < this.frame.frameCount) {
        ...
        this.frameSourceX =
          (frameId * this.frame.width) % this.frame.totalWidth;
        this.frameSourceY =
          parseInt((frameId * this.frame.width) / this.frame.totalWidth) *
          this.frame.height;
      }
    }
  },
...
Object.defineProperties(BoardSprite.prototype, {
  getUpdatedPositionFromPiece: {
    value: function getUpdatedPositionFromPiece() {
      return {
        x:
          this.piece.columnNumber * this.pieceSize +
          this.piece.columnNumber * this.piecePadding,
        y:
          this.piece.rowNumber * this.pieceSize +
          this.piece.rowNumber * this.piecePadding
      };
    }
  },
