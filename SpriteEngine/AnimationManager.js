    ...
    update: {
        value: function update(currentTime) {
            ...
            if (currentTime >= this.startTime) {
                ...
                var offset = 1.0;
                if (this.duration > 0) {
                    offset = Math.min(1.0, (currentTime - this.startTime) / this.duration);
                }
...
function OpacityAnimation(sprite, startOpacity, endOpacity, delay, duration, callbackComplete) {
...
function TranslationAnimation(sprite, sx, sy, dx, dy, delay, duration, callbackComplete) {
    this.sx = sx;
    this.sy = sy;
    this.dx = dx;
    this.dy = dy;
    ...
    if (duration === 0) {
        ...
        realDuration = Math.sqrt(this.deltaX * this.deltaX + this.deltaY * this.deltaY);
    }
...
function ArcAnimation(sprite, sx, sy, deltaX, deltaY, delay, duration, callbackComplete) {
    this.sx = sx;
    this.sy = sy;
...
function SwapAnimation(engine, p1Info, p2Info, delay, duration, callbackComplete) {
...
function KeyFrameAnimation(keyFrameSprite, delay, duration, callbackComplete, options) {
    Animation.call(this, keyFrameSprite, delay, duration, callbackComplete, options);
...
Object.defineProperties(KeyFrameAnimation.prototype, {
    ...
    updateCore: {
        value: function updateCore(offset) {
            var newFrameId = this.startFrame + parseInt(this.frameCount * offset) * this.frameOffset;
