...
Object.defineProperties(Connect3Engine.prototype, {
    ...
    processPieceClick: {
        value: function processPieceClick(columnNumber, rowNumber) {
            ...
            if ((this.board.selectedPieceArray[0] == null) && (this.board.selectedPieceArray[1] == null)) {
                ...
                if ((this.board.selectedPieceArray[0] == null) && (this.board.selectedPieceArray[1] != null)) {
                    ...
                    if (this.board.selectedPieceArray[0] == data) {
                        ...
                        if (this.board.boardType === "hex") {
                            var columnDiff = parseInt(data.columnNumber) - parseInt(this.board.selectedPieceArray[0].columnNumber);
                            var rowDiff = parseInt(data.rowNumber) - parseInt(this.board.selectedPieceArray[0].rowNumber);

                            if (
                               (columnDiff == 0) && (Math.abs(rowDiff) == 1) ||
                               (rowDiff == 0) && (Math.abs(columnDiff) == 1) ||
                               ((parseInt(data.columnNumber) % 2 == 1) && (Math.abs(columnDiff) == 1) && (rowDiff == -1)) ||
                               ((parseInt(data.columnNumber) % 2 == 0) && (Math.abs(columnDiff) == 1) && (rowDiff == 1))
                              ) {

                                //1. first, swap and register swap in the action array
                                var piece1ColumnNumber = parseInt(this.board.selectedPieceArray[0].columnNumber);
                                var piece1RowNumber = parseInt(this.board.selectedPieceArray[0].rowNumber);
                                var piece2ColumnNumber = parseInt(this.board.selectedPieceArray[1].columnNumber);
                                var piece2RowNumber = parseInt(this.board.selectedPieceArray[1].rowNumber);

                                //register
                                this.boardActionArray.push(new SwapAction(this.board.selectedPieceArray[0], this.board.selectedPieceArray[1]));

                                //swap
                                var piece1Copy = this.board.columnArray[piece1ColumnNumber][piece1RowNumber];
                                this.board.columnArray[piece1ColumnNumber][piece1RowNumber] = this.board.columnArray[piece2ColumnNumber][piece2RowNumber];
                                this.board.columnArray[piece2ColumnNumber][piece2RowNumber] = piece1Copy;

                                this.board.columnArray[piece1ColumnNumber][piece1RowNumber].columnNumber = piece1ColumnNumber;
                                this.board.columnArray[piece1ColumnNumber][piece1RowNumber].rowNumber = piece1RowNumber;
                                this.board.columnArray[piece2ColumnNumber][piece2RowNumber].columnNumber = piece2ColumnNumber;
                                this.board.columnArray[piece2ColumnNumber][piece2RowNumber].rowNumber = piece2RowNumber;
    ...
    turnColorAToColorB: {
        value: function turnColorAToColorB(colorOriginal, colorBecome, updatedPieceInfoArray/*out*/) {
            ...
            for (var i = 0; i < pieceArray.length; i++) {
                for (var j = 0; j < pieceArray[i].length; j++) {
                    if ((pieceArray[i][j] != null) && (pieceArray[i][j].colorCode == colorOriginal)) {
        ...
        value: function getCascadeDirection() {
            ...
            if (this.board.boardType === "square") {
                ...
                if (piece1.columnNumber % 2 == 0) {
                    if ((piece2.columnNumber == piece1.columnNumber) && (piece2.rowNumber > piece1.rowNumber)) {
                        this.cascadeDirection = "vertical-TTB";
                    }
                    else if ((piece2.columnNumber == piece1.columnNumber) && (piece2.rowNumber < piece1.rowNumber)) {
        ...
        value: function updateBoard(boardActionArray) {
            ...
            do {
                ...
                if (deadPieceArray.length > 0) {
                    ...
                    boardActionArray.push(new DeadPieceAction(cascadeCount, deadPieceArray, this.deadPieceScores));
    ...
    markOffPiecesFromArray: {
        value: function markOffPiecesFromArray(cascadeCount) {
            ...
            if (typeof (this.erasingSingleColor) === "undefined") {
                ...
                var endIndexArray;
                if (this.board.boardType === "hex") {
                    directionArray = [Iterator.TYPE_VERT_DOWN, Iterator.TYPE_DIAG_LEFTTORIGHT_FORWARD, Iterator.TYPE_DIAG_RIGHTTOLEFT_FORWARD];
                    endIndexArray = [this.board.columnCount, this.board.rowCount + parseInt(this.board.columnCount / 2), this.board.rowCount + parseInt((this.board.columnCount - 1) / 2)];
                }
                else {
                ...
                for (var i = 0; i < pieceArray.length; i++) {
                    for (var j = 0; j < pieceArray[i].length; j++) {
                        if ((pieceArray[i][j] != null) && (pieceArray[i][j].colorCode == this.erasingSingleColor)) {
                            pieceArray[i][j].markDead(this);
                            deadPieceArray.push(pieceArray[i][j]);
                        }
                    }
                }
                this.deadPieceScores.push({ "lastColor": this.erasingSingleColor, "currentCount": deadPieceArray.length });
        ...
        value: function getCompactInfo() {
            ...
            switch (this.cascadeDirection) {
                ...
                case "diagnol-LTR":
                    ...
                    compactInfo.startIndex = 0;
                    compactInfo.endIndex = this.board.rowCount + parseInt((this.board.columnCount - 1) / 2);
                    break;
                ...
                case "counterDiagnol-RTL":
                    ...
                    compactInfo.startIndex = 0;
                    compactInfo.endIndex = this.board.rowCount + parseInt((this.board.columnCount) / 2);
                    break;
            };
    ...
    compactBoardColumnArray: {
        value: function compactBoardColumnArray(boardActionArray, cascadeCount) {
            ...
            boardActionArray.push(new UpdatePieceAction(cascadeCount, slideInPieceArray));
        }
    },
    fillNewPieces: {
        value: function fillNewPieces(boardActionArray, cascadeCount) {
            ...
            for (var i = compactInfo.startIndex; i < compactInfo.endIndex; i++) {
                ...
                while (iter.moveNext()) {
                    var curPiece = iter.Current();
                    if (curPiece === null) {
                        curPiece = this.createPiece(iter.currentCol, iter.currentRow, this.board);
